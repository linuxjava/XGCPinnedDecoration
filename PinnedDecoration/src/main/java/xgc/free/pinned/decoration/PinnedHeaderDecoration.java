package xgc.free.pinned.decoration;
...
public class PinnedHeaderDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        if (mPinnedHeaderView != null) {
            ...
            //判断这个ItemView是否满足PinnedHeader的条件
            ...
            //先对已绘制的recycview视图进行裁剪，裁剪去掉mPinnedHeaderView，保留其它
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        if (mPinnedHeaderView != null) {
            ...
            //将mPinnedHeaderView重新绘制到顶部，这样PinnedHeader就固定头的展示，只有当下一个PinnedHeader滑动过来时会被移动（通过mPinnedHeaderTop进行控制）
...
//        int firstVisiblePosition = ((RecyclerView.LayoutParams) layoutManager.getChildAt(0).getLayoutParams()).getViewAdapterPosition();
        ...
        if (headerPosition >= 0 && mHeaderPosition != headerPosition) {
            ...
            //找到headerPosition所对应的itemView视图，并将这个视图保存在mPinnedHeaderView
    ...
    /**
     * 第一次初始化或DataChanged时，需要resetPinnedHeader
     * @param parent
     */
    ...
    public PinnedHeaderDecoration registerTypePinnedHeader(int itemType, PinnedHeaderCreator pinnedHeaderCreator) {
